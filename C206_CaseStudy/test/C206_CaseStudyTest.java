...
public class C206_CaseStudyTest {
	...
	//Akmal
		private RegisterCCA sC1;
		private RegisterCCA sC2;
		
		private ArrayList<RegisterCCA> studentCCAList = new ArrayList<RegisterCCA>();
...
public void setUp() throws Exception {
	...
	sC1 = new RegisterCCA("21033695" , "Akmal", "jewellery making");
	sC1 = new RegisterCCA("21032795" , "Ahmad", "Little league soccer");
}
	...
	//Done by Jia Xin
	 ...
	 @Test
	  ...
	  public void testAddStudentCCA() {
		  //test if studentCCAList is not null
		  assertNotNull("Test if there is valid arraylist to add to", studentCCAList);
		  
		  //Test that if added one student in the list, the size of the list is 1
		  C206_CaseStudy.addStudentCCA(studentCCAList, sC1);
		  assertEquals("Test if the arraylist size is 1?", 1, studentCCAList.size());
		
		  //Test that if added one student in the list, the student is the same as the first student of the list
		  C206_CaseStudy.addStudentCCA(studentCCAList, sC1);
		  assertEquals("Test if the student added is the same as the first student on the list?", sC1, studentCCAList.get(0));
		  
	  }
	  @Test
	  
	  public void testretrieveStudentCCA() {
		  //test if the list is not null
		  assertNotNull("Test if there is valid arraylist to add to", studentCCAList);
		  
		  //Test if the expected string is the same as every string
		  String studentCCA = C206_CaseStudy.retrieveStudentCCA(studentCCAList);
			String testOutput = "";
			assertEquals("Check that ViewAllCamcorderlist", testOutput, studentCCA);
			
			//Test if the size of the list is 2 when there are 2 students on the list
			C206_CaseStudy.addStudentCCA(studentCCAList, sC1);
			C206_CaseStudy.addStudentCCA(studentCCAList, sC2);
			assertEquals("Test if the arraylist size is 2?", 2, studentCCAList.size());
